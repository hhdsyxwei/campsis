...
### 3.2 State Management Interfaces
...
### 4.2 Global Interfaces

- `download_xrxd(db_conn, start_year, end_year)`: Download interface provided externally
- `download_xrxd_from_scratch(db_conn, start_year, end_year)`: Download from scratch interface provided externally

### 4.3 State Management Interfaces
...
5. Loop through each download task
6. Update download pointer before each download
7. Get next task after download completion
8. Clear download pointer after all tasks completed
...
### 6.1 Resume from Breakpoint Flow
1. Call `download_xrxd` method
...
6. Execute download tasks one by one
7. Clear download pointer after all tasks completed

### 6.2 Download from Scratch Flow
1. Call `download_xrxd_from_scratch` method
2. Delete task record
3. Call `download_xrxd` method (status is now "Download Not Started")
4. Start downloading all tasks from scratch
